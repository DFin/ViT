...
(Ascii image credits mostly ChatGPT)
...
if __name__ == '__main__': # this is needed for multiprocessing
    ...
    class Block(nn.Module):
        ...
        def __init__(self, n_embd, n_head):
            ...
            self.ln1_x2 = nn.LayerNorm(n_embd)

        def forward(self, x, x2=None):
            if x2 == None:
                x = x + self.attn(self.ln1(x))
            else:
                x = x + self.attn(self.ln1(x), self.ln1_x2(x2))
    ...
    class Transformer(nn.Module):
        ...
        def __init__(self):
            ...
            self.enc_blocks_1 = nn.ModuleList([Block(n_embd, n_head) for _ in range(n_layers)])
            self.enc_blocks_2 = nn.ModuleList([Block(n_embd, n_head) for _ in range(n_layers)])
            self.dec_blocks = nn.ModuleList([Block(n_embd, n_head) for _ in range(n_layers)])
        ...
        def forward(self, x, y=None):
            ...
            x_in = x # save input for later use in cross attention
            
            for i in range(n_layers):
                x = self.enc_blocks_1[i](x)
                x2 = self.enc_blocks_2[i](x_in)
                x = self.dec_blocks[i](x, x2)
            x = self.ln_f(x) # final layer norm
            logits = self.linear_f(x) # final linear layer (B, n_embd) -> (B, n_classes)
